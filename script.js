...
window.onload = function() {
  ...
  document.getElementById("footprintForm").onsubmit = function(e) {
    ...
    let v5 = document.getElementById('q5').value * 1;
    ...
    addDoc(collection(database, 'simulatorScores'), { 
        score: summ, 
        date: serverTimestamp() /* using server time so broken laptops dont break it */ 
    }).then(function(){
        // saved successfully
    }).catch(function(err){
  ...
  const qqq = query(refs, orderBy("timestamp", "desc")); /* sorts newest first so it looks active */
  ...
  onSnapshot(qqq, function(s) {
      ...
      s.forEach(function(d){
          ...
          if(o.status == "claimed"){
              ...
              var html = "<div class='item-card' id='card-" + iid + "'>";
