...
/*eco-shift habit transformer graph logic. index found chart.js on a blog and we are basically data scientists now*/ let ecoChart = null; document.getElementById('habitFreq').addEventListener('input', (e) => { document.getElementById('freqDisplay').innerText = e.target.value; }); document.getElementById('calcImpactBtn').addEventListener('click', () => { let h = document.getElementById('habitSelect').value; let f = parseInt(document.getElementById('habitFreq').value, 10); if(!h){ alert("bro pick a habit first smh"); return; } let cost = 0; let co2 = 0; if(h === "water"){ cost = 20; co2 = 0.08; } else if(h === "coffee"){ cost = 150; co2 = 0.11; } else if(h === "meat"){ cost = 300; co2 = 3.3; } else if(h === "drive"){ cost = 50; co2 = 1.2; } let savedMoney = cost * f * 52; let savedCO2 = Math.round(co2 * f * 52); document.getElementById('moneySaved').innerText = "₹" + savedMoney.toLocaleString("en-IN"); document.getElementById('co2Saved').innerText = savedCO2 + "kg"; document.getElementById('chartBox').style.display = "block"; if(ecoChart){ ecoChart.destroy(); /*destroy the old chart because otherwise they overlap when you click again and it looks like a glitch matrix*/ } let ctx = document.getElementById('impactChart').getContext('2d'); ecoChart = new Chart(ctx, { type: 'bar', data: { labels: ['1 Month', '6 Months', '1 Year'], datasets: [{ label: 'CO2 Prevented (kg)', data: [Math.round(savedCO2/12), Math.round(savedCO2/2), savedCO2], backgroundColor: '#2ecc71', borderColor: 'black', borderWidth: 3 }] }, options: { responsive: true, plugins: { legend: { labels: { font: { family: "'Space Grotesk', sans-serif", weight: 'bold' } } } }, scales: { y: { beginAtZero: true, grid: { color: 'black' }, ticks: { font: { family: "'Space Grotesk', sans-serif", weight: 'bold' }, color: 'black' } }, x: { grid: { color: 'black' }, ticks: { font: { family: "'Space Grotesk', sans-serif", weight: 'bold' }, color: 'black' } } } } }); });
