import /*import firebase stuff idk what this is tysm yt tutorials*/ { 
    initializeApp 
} from "https://www.gstatic.com/firebasejs/12.16.0/firebase-app.js"; /*this works*/
...
}; /*if you change a single quote mark in here the entire database crashes this is not safe pls hacker if seein this dont steal our projct */
...
document.getElementById('footprintForm').addEventListener('submit', async function(event) {
    ...
    let q1 = parseInt(document.getElementById("q1").value, 10) || 0; /*grabbing the answers and converting to real numbers bcuz javascript is dumb*/
    let q2 = parseInt(document.getElementById("q2").value, 10) || 0; /*using base 10 because index learned that today*/
    let q3 = parseInt(document.getElementById("q3").value, 10) || 0;
    let q4 = parseInt(document.getElementById("q4").value, 10) || 0; /*index ran out of good variable names*/
    let q5 = parseInt(document.getElementById("q5").value, 10) || 0; /*a5 is apiece of paper*/
    ...
    if (totalScore >= 80) { /*judging the user heavily based on score*/
        feedbackEmoji = "🌍";
        barColor = "green";
        feedbackTextElement.innerText = "🔥 INCREDIBLE! You implemented a true sustainable framework. By shifting to renewables and enforcing a circular economy, we can reach Net-Zero!";
        feedbackTextElement.style.color = "green";
    } else if (totalScore >= 40 && totalScore < 80) {
        feedbackEmoji = "⚠️";
        barColor = "orange";
        feedbackTextElement.innerText = "🌱 A GOOD START. But half-measures aren't enough. We need systemic shifts in lots of things. Try again!";
        feedbackTextElement.style.color = "orange";
    } else {
        feedbackEmoji = "❌"; /*literal doomsday scenario*/
        barColor = "red";
        feedbackTextElement.innerText = "🚨 DISASTER. Continuing the status quo guarantees severe global warming. We need massive policy shifts immediately.";
        feedbackTextElement.style.color = "red";
    }

    document.getElementById("resultEmoji").innerText = feedbackEmoji; /*tysm stackoverflow again*/
    ...
    let scoreCounter = setInterval(function() { /*ok so index watched a 40 minute youtube video just to make this number count up it looks so professional tho the judges are gonna love it*/
        if (currentScore >= totalScore) {
            clearInterval(scoreCounter); /*stop counting or it goes to infinity*/
    ...
    }, 150); /*wait a tiny bit then slide the bar across pure css magic combined with js index am a literal hacker*/
});

/*some logic*/
const boardCollection = collection(db, "listedItems");
const boardQuery = query(boardCollection, orderBy("timestamp", "desc")); /*sorts newest first so it looks active or it should*/
...
onSnapshot(boardQuery, (snapshot) => { /*this is basically magic it just knows when stuff changes kinda creepy tbh*/
    ...
    let claimedList = document.getElementById('claimed-list'); /*the dropdown list thats made*/

    liveBoard.innerHTML = ""; /*empty these out or it duplicates forever it happened at 11pm and my laptop crashed*/
    ...
    if (itemCount === 0) {
        liveBoard.innerHTML = "<h3 style='width:100%;text-align:center;color:gray;'>No items available right now. Be the first to list something!</h3>";
    } /*if the database is empty it looks dumb so we put a placeholder message so dont seem dumb*/
    
    if (claimedCount === 0) {
        claimedList.innerHTML = "<li>No items claimed yet... be the first!</li>";
    }
});
...
document.getElementById('addItemForm').addEventListener('submit', async(event) => { /*submit form for new item this part is kinda cool actually*/
    ...
    let submitBtn = document.querySelector(".post-btn");
    let originalText = submitBtn.innerText;
    submitBtn.innerText = "UPLOADING..."; /*make the button look like its thinking so kids dont spam click it*/
...
window.claimIt = function(itemId) { /*putting this on window so the inline html onclick can actually see it if you dont do this the button literally says function not found and you look like an idiot*/
    ...
    setTimeout(function() { /*fake delay so it looks like its doing hard math before updating the database*/
        updateDoc(doc(db, "listedItems", itemId), {
            status: "claimed",
            claimedBy: userName
        }).catch((error) => {
